...
import BlogPostPreview from "../../news/blog-post-preview";
...
function NewsCarousel(props) {
  ...
  let filteredPosts = {};
  filteredPosts = guides.allSanityPost.edges.filter(node => {
    if (catID) {
        ...
        <div className="reachers_slider">
          ...
          {catID && (
            <div className="see_more">
              <Link to={`/${props.category.slug.current}/news`}>
                View all {props.category.title} Articles
              </Link>
            </div>
          )}
        </div>
